...
$$
  ...
  $$
The useful differential relations follow as
$$
...
$$f \coloneqq q^2 + M p(p-p_\text{c}) \leq 0 \ .
$$
Here, $M$ is the
...
used to obtain the plastic flow as $^4$
$$
...
compression -- evolves:
$$
  \dot{p}_\text{c}= -\dot{\varepsilon}_\text{p}^\text{V} \vartheta(v)\ p_\text{c} \quad \text{with}\quad p_\text{c}|_{t=0} = p_{\text{c}0} \ .
$$
...
$$\vartheta(v) = \frac{v}{\lambda - \kappa} = \frac{1 + e}{\lambda - \kappa} =  \frac{1}{(\lambda - \kappa)(1 - \phi)} = \vartheta(\phi) \ ,$$

where the material constants $\lambda, \kappa$ ($\lambda>\kappa$) are
unit-less. This leads to
$$
...
normal swelling line, respectively.

With the porosity (or, equivalently, pore number or volume ratio)
evolution, the system of constitutive equations for
...
a geometrically linear setting this means that $v\approx v_0$. The linearized evolution of the volume
ratio reads
$$
  \Delta v = v_0\,\Delta\varepsilon^\text{V} \ll 1 \ .$$
  The evolution
equations derived previously now become
$$
  ...
  \ .$$ 
  and can be solved analytically as well. Thus, the total volume strain rate reads
...
The evolution equations imply that the elastic bulk modulus
...
is chosen). The linearized hypoelastic
relation
 for the pressure evolution can be solved
...
Hooke's law over a time increment
...
\begin{align}
  ...
  f_{\!\varLambda_p} &= q^2 + M^2(p^2 - p\,p_\text{c}) = 0 \ , \\ %\frac{1}{E^2}
  f_{p_\text{c}} &= \varDelta p_\text{c}+ \varDelta\varepsilon_\text{p}^\text{V} \vartheta(\phi)\ p_\text{c}= 0 \ , \\
  f_{\phi} &= \varDelta\phi - (1-\phi) \varDelta\varepsilon^\text{V} = 0 \ , 
\end{align}$$ 
where the total values are the values at the next instant
...
For the solution of the incremental set of equations with the Newton-Raphson method the
...
\end{align}$$ 
All other partial derivatives vanish according to the
(missing) dependencies. Using the normalized flow
...
\end{aligned}$$ 
Now, the missing expressions can be calculated as
$$\begin{align}
...
\end{align}$$ 
The solution can be accomplished based on the
...
known as active set search). Alternatively, the case
...
## Numerical refinement and stabilization

It is recommended to normalize all residuals
 to some similar order of magnitude,
e. g. as strains. Consider
$$\begin{aligned}
...
\end{aligned}$$ 
Here $\hat{f} = E\,\hat{p}_\text{c}= E\,p_{\text{c}0}$
...
corresponding partial derivatives. Instead of applying the same
...
possible because then the elastic parameters take unphysical values. This is no
...
but plastic flow is undetermined. To prevent this case, a small
(ambient) pressure $p_\text{amb}$ can be added to the hydrostatic
pressure, i. e.
$$p \coloneqq -I_1(\underline{\sigma})/3 + p_\text{amb} \ .$$
Hence, a
...
evolution equation is modified to 
$$
  \dot{p}_\text{c}= -\dot{\varepsilon}_\text{p}^\text{V} \vartheta(\phi)\ (p_\text{c}- p^\text{min}_\text{c}) \quad\text{with}\quad p_\text{c}|_{t=0} = p_\text{c0} \ ,
$$
...
The number of equations can be reduced exploiting the minor
...
evolution: the volume ratio itself then follows the
linear evolution
$${}^{k+1}v = {}^{k}v + v_0 \Delta\varepsilon^\text{V} 
  \quad\text{with}\quad v|_{t=0} = v_{0} \ .
  $$
  In all other
...
equation. Thus, the corresponding residual equation and
...
Integrating the non-linear relation
 between volume ratio and volumetric strain
...
the time step [@Borja1990].

## Forcing linear elastic behavior with constant parameters {#subsec:linear_elastic}

The evolution equation for the hydrostatic pressure causes a
...
ratio/pore number. For these reasons and for the sake of
simplicity, constant elastic parameters can be forced. This means
 $$
  \dot{p} = -\dot{\varepsilon}_\text{e}^\text{V}\ K \ ,
  $$ 
  which is
...
$$
Nevertheless,
the hardening behaviour will be a bit different.

# Implementation into *MFront*

## MCC model with constant elasticity
...
## MCC model with original non-linear elasticity
...
## Isotropic compression and consolidation line test using *mtest*
...
$E$ cannot be chosen arbitrarily, but according to 
$$E = 3(1-2\nu) K = v_0\cdot 3(1-2\nu) \frac{p_0}{\kappa} = 3 \frac{(1-2\nu)}{(1-\phi_0)} \frac{p_0}{\kappa} \ .$$

Furthermore, no ambient pressure is used.

<figure id="fig:Comparison_NCL_MCC_OCR">
<img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/Comparison_NCL_MCC_new_OCR=4.png"
style="width:80.0%" />
...
The figures shows the perfect fit of
...
elastic parameters. However, the version
 with constant elastic
parameters significantly deviates. Hence, the
...
## Consolidated plane strain simple shear test using *mtest*
...
<p><img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_ParamStudy_ShearCurves_pStudy.png"
style="width:60.0%" /> <img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_ParamStudy_eplVCurves_pStudy.png"
style="width:60.0%" /></p>
...
<img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_ParamStudy_YieldSurface_pStudy.png"
style="width:61.0%" />
...
<p><img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_ParamStudy_ShearCurves_pcStudy.png"
style="width:52.0%" /> <img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_ParamStudy_eplVCurves_pcStudy.png"
style="width:52.0%" /> <img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_ParamStudy_YieldSurface_pcHigh.png"
style="width:52.0%" /> <img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_ParamStudy_YieldSurface_pcLow.png"
style="width:52.0%" /></p>
...
As can be seen the materials strive to the same
...
using *OpenGeoSys*, but without consolidation
...
parameters were taken as previously with only one difference: As the test
has no pre-consolidation phase, it starts from zero stress some small ambient pressure
...
<figcaption>Test results for different BCs: the top boundary
...
previous section. The table shows under which conditions convergence
...
minimal value $p^\text{min}_\text{c}$ has to be ensured.
...
cases from test number 6.</figcaption>
</figure>
...
from the same settings are applied in
order to compare the *original* Modified Cam Clay model
and the implementation with
constant elastic parameters. The Young's modulus is thus
...
class="math inline"><em>x</em></span>) direction. </figcaption>
</figure>
...
trajectory approaches the CSL from the right side but does not meet it. Otherwise there will be zero
...
can already be seen with the steep
increase of the equivalent plastic strain. The curve of the
pre-consolidation pressure shows monotonic
hardening related to plastic compaction.
...
<figcaption>Triaxial benchmark results using implementation Original: shown is the
...
solutions are not identical but close to each other. The
reason is the pronounced deviatoric stress component -- in contrast to
the pure isotropic compression test.

<figure id="fig:triaxStressTrajectory">
<img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_TriaxStudy_YieldSurface_41.png"
style="width:70.0%" />
<figcaption>Triaxial benchmark results using implementation Const E: depicted is the
...
. For this, the straight stress
path starting from $(p, q) = (p_{\text{c}0}, 0)$ until the final state
$(p, q)=(387387, 330129)\ $Pa is considered. Plotting the von-Mises stress
...
<p><img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_TriaxStudy_Strains_41.png"
style="width:52.0%" /> <img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_TriaxStudy_NumVsAnal_41.png"
style="width:52.0%" /></p>
<figcaption>Triaxial benchmark results using implementation Const E: depicted are the
...
<p><img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_TriaxStudy_Strains_42.png"
style="width:52.0%" /> <img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/ModCamClay_TriaxStudy_NumVsAnal_42.png"
style="width:52.0%" /></p>
...
is found: The initial plastic compaction
...
In order to check the convergence rate of the Cam clay implementation the consolidated shear test
 was considered again. The hydrostatic pressure $p$ was
...
<img
src="img/SemiExplicitModifiedCamClay_OpenGeoSys2023/convergence_plot.png"
style="width:104.0%" />
...
modified Cam clay <em>MFront</em> implementation and <em>mtest</em>.
...
Convergence is achieved in one step in
...
Implementation of the modified Cam clay model
...
Since for Implementation the standard elasticity brick
...
$$
  1-\phi = (1-{}^{0}\!\phi) \exp(-\varepsilon^\text{V}) \ .
  $$ 
  This finally yields the evolution of the
...
and for the reasons explained previously, the volume ratio is assumed
almost constant. Including the minimum value, solving by separation of variables and
...
corresponding partial derivatives. In a similar way, the
*analytical* Solution for the pressure evolution can
...
$$q = k\,(p-p_{\text{c}0}) \ .$$ 
The analytical solution [@Peric2006]
...
## Analytical considerations on pressure-dependent hypoelasticity
...
Assuming the volume ratio almost constant, the dependency
...
for the pressure evolution can be solved
analytically. With this calculation of the
...
$^1$: Original version of this document at:\
    <https://www.opengeosys.org/docs/benchmarks/small-deformations/modifiedcamclay/ModifiedCamClay_report.png>
