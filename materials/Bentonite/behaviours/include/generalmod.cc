...
long Hypoplasti_unsat_expansive_thermal::soil_model(double strain_gen[], double stress_gen[],
  ...
  //direct stiffness matrix (approximate, but can be calculated with zero strain increment provided)
    ...
    errorstate += correct_statev_values(strain_gen, stress_gen, qstatev, dstrain_gen_zero, 1);
  ...
  //Updates variables, if flag == 1 also stiffness matrix by perturbation
    ...
    errorstate += correct_statev_values(strain_gen, stress_gen, qstatev, dstrain_gen, 1);
    ...
    errorstate += correct_statev_values(strain_gen, stress_gen, qstatev, dstrain_gen, 2);
    
    //Calculate stiffness matrix by perturbation of current strain increment
    if(flag == 1) {
      bool perturbate=true;
      if(kinc>-2) perturbate=false;
      ...
      if(perturbate) {
        ...
        for(int i=0; i<ngstrain; i++) {//stiffness matrix as dDsig/dDepsiolon
          ...
          errorstate += correct_statev_values(strain_gen_pert, stress_gen_pert, qstatev_pert, dstrain_gen_pert, 3);
      ...
      errorstate += correct_DDtan_gen(strain_gen, stress_gen, DDtan_gen);
    }
  ...
  long chyba = 0;
  if(check_math_error()) chyba = 1;
  if(errorstate) chyba += 2;
  if(errorrkf) chyba += 4;
  if(chyba) cout<<"Generalmodel ended up with error = "<<chyba<<endl;
  return (chyba);
}
...
bool Hypoplasti_unsat_expansive_thermal::calc_fsigq(double signet[9], double suction, double Temper,
  ...
  */
  ...
  double rlambda=give_rlambda(suction, a_scan, dsuction, sewM, SrM, rlambda_for_ascan);
  ...
  //calculation of the double structure coupling factor
    ...
    fm=(csh*(suction/sewM));
    if(swelling)
      fm=1-gscalar_power(re,mmparam); //swelling
    if(fm>1)
      fm=1;
    else if(fm<0)
      fm=0;
  ...
  //Calculate total stress rate from the effective stress rate of Macrostructure
    double corr_s=(1-lambdap0*rlambda)*dsuction;
    double deM=(1+eM)*(gtrace(DM)+(fm-1)*trdepm);
    double corr_eM=-lambdap0*suction*deM/eM;
    double corr_T=+lambdap0*suction*bTparam*dTemper/(aTparam+bTparam*Temper);
    if(suction>sewM)
      corr_eM=corr_T=0;

    for(int i=0; i<3; i++) {
      for(int j=0; j<3; j++) {
         dsignet_iter[3*i+j]=dsigMef[3*i+j]-kron_delta[i][j]*SrM*(corr_s+corr_eM+corr_T);
      }
    }

    if(eM<0.0001) {//for zero eM all the deformation is due to microstructure
      fu=0;
      garray_set(depM,0,9);
      fm=1;
...
void Hypoplasti_unsat_expansive_thermal::make_sig_unsat(double sig[3*3], double suction, double SrM, 
  ...
  tensor_unsat[0]+=chi*scalar_unsat;
  tensor_unsat[4]+=chi*scalar_unsat;
  tensor_unsat[8]+=chi*scalar_unsat;
}
...
void Hypoplasti_unsat_expansive_thermal::calc_depm(double signet[9], double suction, double Temper, double dsuction,
  ...
  if((depm[0]+depm[4]+depm[8])>0) {
    for(int i=0; i<3; i++) depm[3*i+i]*=(1-fu);
  }
}
...
//        cout<<"OCR="<<peast/p_mean<<endl;
  ...
  double phimfactor=-8*I3/(I3+I1*I2);
...
double Hypoplasti_unsat_expansive_thermal::give_SrM_new(double suction, double SrM, double eM, double Temper, double sewM, double se, double dsuction, double deM, double dTemper, double a_scan, int flag) {
  ...
  if(flag==0) {
    ...
    double dSrM_ds=0;
    if(suction<-1.0) dSrM_ds=-lambdap0/suction*SrM;
    else dSrM_ds=-lambdap0/-1*SrM;
    double dSrM_dse=lambdap0*SrM/se;
    ...
    double dSrM_main=dSrM_ds*dsuction+dSrM_dse*(dse_deM*deM+dse_dT*dTemper);
    double rlambda_for_ascan=0;
    double rlambda=give_rlambda(suction, a_scan, dsuction, sewM, SrM, rlambda_for_ascan);
    double dSrM=dSrM_main*rlambda;
...
double Hypoplasti_unsat_expansive_thermal_original::give_rlambda(double suction, double a_scan, double dsuction, double sewM, double SrM, double& rlambda_for_ascan) {
...
double Hypoplasti_unsat_expansive_thermal::give_rlambda(double suction, double a_scan, double dsuction, double sewM, double SrM, double& rlambda_for_ascan) {
  ...
  if(SrM>1) SrM=1;
  if(suction>=sewM) rlambda=0; //drying from 0
  rlambda_for_ascan=rlambda;
  
  if(dsuction>0 && SrM>SrMlimit) rlambda=gscalar_power((1-SrM)/(1-SrMlimit), WRCpower); //wetting to 0
...
void Hypoplasti_unsat_expansive_thermal::direct_stiffness_matrix(double strain_gen[], double stress_gen[], double qstatev[], 
    ...
    for(int i=0; i<c_ngstrain; i++) {//stiffness matrix by perturbation
    ...
    if(perturbatestiffness) {
      for(int i=0; i<6; i++) {
        for(int j=0; j<6; j++) {
          DDtan_gen[i*c_ngstrain+j]=DDtan_gen[i*c_ngstrain+j];
        }
      }
    }
    else {
...
long General_model::calc_rkf(double strain_gen[], double stress_gen[], double qstatev[], double dstrain_gen[], double dtime,
    ...
    if(rkf_parms==NULL) {
      double rkf_parms_default[nrkf_parms] = {1.0e-5,1.0e-17,10000,1.0e-3,3};
...
double General_model::gscalar_power( double a, double b ) {
  ...
  {
    if(debug) cout<<"Attempting to calculate square root of a negative number="<<a<<endl;
    return(0.0);
  }
}
...
long Hypoplasti_unsat_expansive_thermal::correct_statev_values(double strain_gen[], double stress_gen[], 
  ...
  long errorstate = 0;
  for(int i=0; i<ngstrain; i++) errorstate = isnan( strain_gen[i] );
  for(int i=0; i<ngstrain; i++) errorstate = isnan( dstrain_gen[i] );
  for(int i=0; i<ngstress; i++) errorstate = isnan( stress_gen[i] );
  for(int i=0; i<nstatev; i++) errorstate = isnan( qstatev[i] );
  ...
  if(sigef[0]>-pt) {
      printstat=true;
      signet_corrected[0]=-suction*SrM-pt;
  }
  if(sigef[4]>-pt) {
      printstat=true;
      signet_corrected[4]=-suction*SrM-pt;
  }
  if(sigef[8]>-pt) {
      printstat=true;
      signet_corrected[8]=-suction*SrM-pt;
  }
  if(debug & printstat & call != 4) cout<<"Correcting signet, call "<<call<<", signet[0]="<<signet[0]<<", signet[4]="<<signet[4]<<", signet[8]="<<signet[8]<<", suction="<<suction<<", dsuction="<<dstrain_gen[6]<<", SrM="<<SrM<<endl;  
...
long Hypoplasti_unsat_expansive_thermal::correct_DDtan_gen(double strain_gen[], double stress_gen[], double *DDtan_gen) {
  ...
  long errorstate = 0; 
  for(int j=0; j<ngstress*ngstrain; j++) if(isnan(DDtan_gen[j])) errorstate += 1;
  return (errorstate);
};
