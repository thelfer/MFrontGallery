...
function(add_mtest interface lib)
  ...
  foreach ( _ARG ${ARGN})
    message(STATUS "treating argument ${_ARG}")
    ...
    elseif ( ${_ARG} MATCHES MATERIAL_PROPERTIES_LIBRARIES )
      ...
      if ( ${_CMD} MATCHES TEST_NAME )
        if (_TEST_NAME)
          message(FATAL_ERROR "add_mtest: test name already specified")
        endif(_TEST_NAME)
        set (_TEST_NAME "${_ARG}")
      elseif ( ${_CMD} MATCHES MTEST_FILE )
        if (_MTEST_FILE)
          message(FATAL_ERROR "add_mtest: mtest file already specified")
        endif(_MTEST_FILE)
        set (_MTEST_FILE "${_ARG}")
  ...
  endif(NOT _TEST_NAME)
  if(NOT _MTEST_FILE)
    set(_MTEST_FILE "${_TEST_NAME}")
  endif(NOT _MTEST_FILE)
	...
	foreach(rm ${IEEE754_ROUNDING_MODES})
	  add_test(NAME ${_TEST_NAME}_${conf}_${rm}_mtest
	    COMMAND mtest --rounding-direction-mode=${rm} --verbose=level0
                      --xml-output=true --result-file-output=false
